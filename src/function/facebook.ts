...
// import model
import facebook_members from "../schema/user-facebook";
...
export default async function Facebook(req: Request, res: Response) {
  ...
  if (!userId || !accessTokenFacebook) {
    ...
    const secret_refreshToken: string = process.env
      .SECRET_REFRESHTOKEN as string;

    try {
      const result = await axios.get(
        `https://graph.facebook.com/v4.0/${userId}?fields=id,name,email&access_token=${accessTokenFacebook}`
      );

      const facebookId = result.data.id;
      const facebookName = result.data.name;
      const existUser = await facebook_members.findOne({
        facebookId: { $eq: facebookId },
      });
